...
| — | etcd restore — edited `--data-dir`/`mountPath` in the static pod manifest instead of the volume's host path, and etcd failed to start | After `etcdctl snapshot restore --data-dir=/var/lib/etcd-restore`, edit `/etc/kubernetes/manifests/etcd.yaml` and change ONLY the `etcd-data` volume's `hostPath.path` to the restore dir — leave `--data-dir` and `volumeMounts.mountPath` at `/var/lib/etcd` (the restored host dir is bind-mounted where etcd already looks). |
